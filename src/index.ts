...
function convertirAAmarillo(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.toyellow(imagenSal));
}
//funcion morado implementa
function convertirAMorado(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.tomorado(imagenSal));
}
//funcion morado implementa
function convertirAgua(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.toagua(imagenSal));
}
// resaltado rojo
function resaltadorojo(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.torrojo(imagenSal));
}
//resaltado verde
function resaltadoverde(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.torverde(imagenSal));
}
//resaltado azul
function resaltadoazul(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.torazul(imagenSal));
}
//convertir colores
function convertirColores(evt: any): void{
  var imagenSal:ImageType = new ImageType(pantalla1, imgLocal.getImage());
  imagenSal.imageArray2DtoData(pantalla2, MathImg.toColores(imagenSal));
}
...
//Codigo para el filtro escala, utiliza algunas variables del efecto lluvia
//Escala 0.5
function inimagen() {
  var imagenSal: ImageType = new ImageType(pantalla1, imgLocal.getImage());
  w = imagenSal.getWidth();
...
function escalacinco(evt: any): void { 
  inimagen();
  escalacin();
}
//Escala 0.8
...
function escalaocho(evt: any): void { 
  inimagen();
  escalaoc();
}
//Escala 1
function escalaun() {
  ...
  // Dibujar la imagen a escala en el canvas
  ctx.clearRect(0, 0, 300, 300);
  ctx.drawImage(imgLocal.getImage(), 0, 0, w, h, centerShift_x, centerShift_y, w*ratio, h*ratio);
}
function escalauno(evt: any): void { 
  inimagen();
  escalaun();
}
//Mosaico 
...
document.getElementById("op-rverde").addEventListener('click', resaltadoverde, false);
document.getElementById("op-razul").addEventListener('click', resaltadoazul, false);
document.getElementById("op-colores").addEventListener('click', convertirColores, false);
