...
export class MathImg {
  ...
  //funcion resaltado verde implementada
  public static torverde(img: ImageType): number[][][] {
    //variable que guarda el arreglo 3d de la imagen de color
    var arrImage = img.getArrayImg();
    //variable donde guardamos la salida
    var sal = this.initArray(img.getWidth(), img.getHeight());
  ...
  //funcion resaltado azul implementada
  public static torazul(img: ImageType): number[][][] {
    //variable que guarda el arreglo 3d de la imagen de color
    var arrImage = img.getArrayImg();
    //variable donde guardamos la salida
    var sal = this.initArray(img.getWidth(), img.getHeight());
    ...
    for (let i = 0; i < img.getHeight(); i++) {
      for (let j = 0; j < img.getWidth(); j++) {
        if (arrImage[0][i][j]<100 && arrImage[1][i][j]< 50 && arrImage[2][i][j]>60) {
        sal[0][i][j] = 0;
        sal[1][i][j] = 0;
        sal[2][i][j] = 250;  
        }
